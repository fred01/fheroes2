...
public:
    ...
    // Returns all the events of the map, regardless of the players they are set for.
    const EventsDate & getAllEventsDate() const
    {
        return vec_eventsday;
    }

    // Returns the rumors set by the map creator. Rumors generated by the engine are not included.
